    ...
    for _ in range(num_q):
        ...
        while attempts < 500:
            ...
            if actual_sub_t == "การสร้างจำนวนจากเลขโดด":
                ...
                min_v, max_v = ans_list[0], ans_list[-1]
                ...
                if is_challenge:
                    ...
                    sol = f"""<span style='color:#2c3e50;'><b>วิธีคิดวิเคราะห์แบบ Step-by-Step (Simulation จำลองเหตุการณ์ด้วยภาพ):</b><br>
                    ...
                    sol = f"""<span style='color:#2c3e50;'><b>วิธีคิดวิเคราะห์แบบ Step-by-Step (พร้อมภาพประกอบ):</b><br>
                    <b>ขั้นตอนที่ 1: หาการปีนสุทธิใน 1 วัน</b><br>
                    👉 1 วัน (ขึ้นและลง) จะปีนได้จริง: {u} - {d} = <b>{net} เมตร</b><br>
                    <b>ขั้นตอนที่ 2: หักระยะในวันสุดท้ายออกก่อน (จุดหลอกสำคัญ!)</b><br>
                    👉 ในวันสุดท้าย ถ้า{animal}ปีนพ้นปากบ่อไปแล้ว มันจะไม่ลื่นตกลงมาอีก! เราจึงต้องแยกคิดวันสุดท้ายออกมาก่อน<br>
                    {svg_well}
                    👉 ระยะทางที่ต้องทนปีนแบบลื่นไถล คือ: {h} - {u} = <b>{h - u} เมตร</b><br>
                    <b>ขั้นตอนที่ 3: คำนวณเวลาที่ใช้ปีนระยะช่วงแรก</b><br>
                    👉 นำระยะทางโซนลื่นไถล หารด้วย การปีนสุทธิ: {h - u} ÷ {net} = <b>{math.ceil((h - u) / net)} วัน</b><br>
                    <b>ขั้นตอนที่ 4: รวมกับวันสุดท้าย</b><br>
                    👉 นำเวลาโซนลื่นไถล ไปบวกกับ วันสุดท้ายที่กระโดดพ้นบ่ออีก 1 วัน: {math.ceil((h - u) / net)} + 1 = <b>{days} วัน</b><br>
                ...
                if is_challenge:
                    ...
                    <b>ขั้นตอนที่ 1: หาความยาวแนวนอนของรูปสี่เหลี่ยมผืนผ้าใหม่</b><br>
                    👉 ถ้านำแผ่นกระดาษมาต่อกันเฉยๆ (โดยไม่ให้ซ้อนทับกัน) จะมีความยาว {N} แผ่น × {S} ซม. = {N*S} ซม.<br>
                    👉 แต่ในความจริงมีรอยที่ซ้อนทับกันอยู่ทั้งหมด {N-1} รอย รอยละ {O} ซม. ➔ ทำให้ความยาวหดหายไป {N-1} × {O} = {(N-1)*O} ซม.<br>
                    👉 ความยาวจริงแนวนอน = {N*S} ซม. - {(N-1)*O} ซม. = <b>{width} ซม.</b><br>
                    <b>ขั้นตอนที่ 2: หาความกว้างแนวตั้งของรูปใหม่</b><br>
                    👉 เนื่องจากกระดาษเป็นสี่เหลี่ยมจัตุรัส ความกว้างด้านแนวตั้งจึงยังคงเท่าเดิมคือ <b>{S} ซม.</b><br>
                    <b>ขั้นตอนที่ 3: คำนวณความยาวเส้นรอบรูปทั้งหมด</b><br>
                    👉 สูตรความยาวรอบรูปสี่เหลี่ยมผืนผ้า = 2 × (กว้าง + ยาว) <br>
                    👉 แทนค่า: 2 × ({S} + {width}) = <b>{ans} ซม.</b><br>
                    <b>ตอบ: {ans} ซม.</b></span>"""
                else:
                    ...
                    👉 นำความยาวลวดทั้งหมดตั้ง หักลบด้วยส่วนที่เหลือทิ้งไม่ได้ใช้: {L} - {leftover} = <b>{L-leftover} ซม.</b><br>
                    <b>ขั้นตอนที่ 2: แบ่งความยาวให้ 4 ด้าน</b><br>
                    👉 คุณสมบัติของสี่เหลี่ยมจัตุรัสคือมี 4 ด้านยาวเท่ากันเสมอ<br>
                    👉 นำลวดที่ใช้จริงไปหารเฉลี่ยให้ 4 ด้าน: ({L-leftover}) ÷ 4 = <b>{side} ซม.</b><br>
                ...
                if is_challenge:
                    ...
                    👉 วันที่สามอ่านไป {f1_2_s} (ครึ่งนึง) แล้วยังต้องอ่านเพิ่มอีก {Y} หน้า จึงจะจบเล่มพอดี<br>
                    👉 แสดงว่า {Y} หน้าที่อ่านทีหลัง ก็คือ 'ครึ่งที่เหลือ' นั่นเอง<br>
                    👉 ยอดหนังสือคงเหลือก่อนเริ่มอ่านวันที่ 3 คือ: {Y} × 2 = <b>{R2} หน้า</b><br>
                    <b>ขั้นตอนที่ 2: ย้อนรอยวันที่ 2</b><br>
                    👉 นำ {X} หน้าที่ถูกอ่าน 'เพิ่ม' ไปในวันที่ 2 มาบวกคืนเข้าไปก่อน: {R2} + {X} = {R2+X} หน้า<br>
                    👉 วันที่สองอ่านไป 1/3 แสดงว่า {R2+X} หน้านี้ คิดเป็น {f2_3_s} ของจำนวนหน้าที่เหลือก่อนอ่านวันที่ 2<br>
                    👉 ยอดหนังสือคงเหลือก่อนเริ่มอ่านวันที่ 2 คือ: ({R2+X} ÷ 2) × 3 = <b>{R1} หน้า</b><br>
                    <b>ขั้นตอนที่ 3: ย้อนรอยวันที่ 1 (หักจากหนังสือทั้งเล่ม)</b><br>
                    👉 วันแรกอ่านไป 1/4 แสดงว่า {R1} หน้าที่เหลือนั้น คิดเป็น {f3_4_s} ของหนังสือทั้งเล่ม<br>
                    👉 จำนวนหน้าหนังสือตั้งต้นทั้งเล่ม คือ: ({R1} ÷ 3) × 4 = <b>{Total} หน้า</b><br>
                    ...
                    👉 ความหมายของ {f_s} คือการแบ่งเงินทั้งหมดออกเป็น {den} ส่วน และใช้ไป {num} ส่วน<br>
                    ...
                    👉 นำเงินก้อนตอนแรกสุดตั้ง ลบด้วยเงินที่ใช้จ่ายไป<br>
...
def generate_cover_html(level, sub_t, num_q, brand_name, is_challenge):
    ...
    <style>
        .cover-inner {{ width: 100%; height: 100%; padding: 50px; box-sizing: border-box; text-align: left; font-family: 'Sarabun', sans-serif; position: relative; background: #ffffff; border: 12px double #2c3e50; }}
        .title {{ font-size: 55px; color: #2c3e50; font-weight: 800; margin: 0; line-height: 1.2; text-transform: uppercase; margin-top: 40px; }}
        .subtitle {{ font-size: 32px; color: #7f8c8d; margin-top: 10px; font-weight: normal; }}
        .grade-badge {{ font-size: 35px; background-color: #e74c3c; color: white; padding: 15px 40px; border-radius: 50px; display: inline-block; font-weight: bold; margin-top: 40px; margin-bottom: 40px; box-shadow: 0 4px 6px rgba(0,0,0,0.2); }}
        .topic {{ font-size: 36px; color: #34495e; font-weight: bold; margin-bottom: 10px; }}
        .sub-topic {{ font-size: 28px; color: {theme_color}; margin-bottom: 30px; font-weight: bold; }}
        .icons {{ font-size: 60px; margin-bottom: 40px; letter-spacing: 15px; }}
        .footer-box {{ position: absolute; bottom: 60px; left: 0; width: 100%; text-align: center; }}
        .details-badge {{ background-color: #2ecc71; color: white; display: inline-block; padding: 15px 40px; border-radius: 12px; font-size: 26px; font-weight: bold; margin-bottom: 15px; box-shadow: 0 4px 6px rgba(0,0,0,0.15); }}
        .brand-text {{ font-size: 18px; color: #7f8c8d; }}
    </style></head><body>
    <div class="cover-inner" style="min-height: 1000px;">
        <h1 class="title">Thailand Math Competition</h1>
        <div class="subtitle">ข้อสอบวิเคราะห์คณิตศาสตร์ (TMC)</div>
        <br>
        ...
        <div class="footer-box">
            ...
            <div class="brand-text">ออกแบบและจัดทำโดย: {brand_name}</div>
        </div>
    </div>
    </body></html>"""
...
if 'ebook_html' in st.session_state:
    st.success(f"✅ โค้ดสมบูรณ์เต็ม 100%! คืนชีพหน้าปก Thailand Math Competition แบบเต็มแผ่น และวาดรูปกราฟิกตาชั่ง/เชือก/สัตว์ครบถ้วนครับ")
    c1, c2 = st.columns(2)
    with c1:
