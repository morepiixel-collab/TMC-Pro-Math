...
# ---------------------------------------------------------
# คืนชีพ แบนเนอร์ด้านบน (ตามรูปวงสีแดงที่คุณขอ)
# ---------------------------------------------------------
st.markdown("""
        ...
        while attempts < 500:
            ...
            if actual_sub_t == "การสร้างจำนวนจากเลขโดด":
                ...
                if is_challenge:
                    ...
                    <b>ขั้นตอนที่ 1: หาความยาวแนวนอนของรูปใหม่</b><br>👉 ถ้านำแผ่นมาต่อกันเฉยๆ (ไม่ซ้อนทับ) จะยาว {N} × {S} = {N*S} ซม.<br>👉 แต่มีรอยซ้อนทับกันทั้งหมด {N-1} รอย รอยละ {O} ซม. ➔ หดหายไป {N-1} × {O} = {(N-1)*O} ซม.<br>👉 ความยาวจริงแนวนอน = {N*S} - {(N-1)*O} = <b>{width} ซม.</b><br>
                    <b>ขั้นตอนที่ 2: หาความกว้างแนวตั้งของรูปใหม่</b><br>👉 ความกว้างยังคงเท่าเดิมคือด้านของจัตุรัส = <b>{S} ซม.</b><br>
                    <b>ขั้นตอนที่ 3: คำนวณความยาวรอบรูปทั้งหมด</b><br>👉 สูตรความยาวรอบรูป = 2 × (กว้าง + ยาว) ➔ 2 × ({S} + {width}) = <b>{ans} ซม.</b><br>
                    <b>ตอบ: {ans} ซม.</b></span>"""
                else:
                    ...
                    sol = f"""<span style='color:#2c3e50;'><b>วิธีคิดวิเคราะห์แบบ Step-by-Step:</b><br>
                    <b>ขั้นตอนที่ 1: หาลวดส่วนที่ถูกใช้งานจริง</b><br>👉 นำความยาวลวดทั้งหมดตั้ง หักลบด้วยส่วนที่เหลือทิ้ง: {L} - {leftover} = <b>{L-leftover} ซม.</b><br>
                    <b>ขั้นตอนที่ 2: แบ่งความยาวให้ 4 ด้าน</b><br>👉 คุณสมบัติของสี่เหลี่ยมจัตุรัสคือมี 4 ด้านยาวเท่ากันเสมอ<br>👉 นำลวดที่ใช้จริงไปหาร 4: ({L-leftover}) ÷ 4 = <b>{side} ซม.</b><br>
...
if st.sidebar.button(f"{'🚀 สั่งสร้างข้อสอบระดับ Ultimate Challenge!' if is_challenge else '🚀 สั่งสร้างข้อสอบแข่งขันเดี๋ยวนี้'}", type="primary", use_container_width=True):
    with st.spinner("กำลังออกแบบข้อสอบ วาดภาพกราฟิกประกอบ และจัดทำเฉลยแบบ Step-by-Step..."):
        ...
        # ⚠️ ส่งตัวแปร 4 ตัวอย่างถูกต้อง (level, sub_t, num_q, is_challenge) แก้บั๊กจอแดง 100%
