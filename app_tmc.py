...
# ---------------------------------------------------------
# ฟังก์ชันวาดภาพประกอบ (SVG/HTML) สำหรับข้อสอบ
# ---------------------------------------------------------
...
comp_db = {
    ...
    "ระดับประถมกลาง (ป.3 - ป.4)": core_topics + [
        ...
        "การคิดย้อนกลับ", "แผนภาพความชอบ", "ผลบวกจำนวนเรียงกัน (Gauss)"
    ],
        ...
        while attempts < 500:
            ...
            if actual_sub_t == "การสร้างจำนวนจากเลขโดด":
                if is_challenge:
                    ...
                    sol = f"""<span style='color:#2c3e50;'><b>วิธีคิดวิเคราะห์แบบ Step-by-Step (เคล็ดลับสร้างผลคูณสูงสุด):</b><br>
                    การจะทำให้ผลคูณมีค่ามากที่สุด เราต้องวางตัวเลขค่ามากไว้ในตำแหน่งที่ไป 'คูณ' กับหลักอื่นๆ ให้ได้ผลประโยชน์สูงสุด (นั่นคือหลักที่อยู่ด้านหน้าสุด)<br>
                    <b>ขั้นตอนที่ 1: เรียงตัวเลขที่เรามีจากมากไปน้อย</b><br>
                    👉 จะได้ตัวเลขคือ: <b>{sd[0]} > {sd[1]} > {sd[2]} > {sd[3]} > {sd[4]}</b><br>
                    <b>ขั้นตอนที่ 2: สร้างสมดุลด้วยการ "คูณไขว้"</b><br>
                    👉 <b>หลักหน้าสุด (หลักร้อย/หลักสิบ):</b> นำเลขสองตัวแรกที่มากที่สุด ({sd[0]} และ {sd[1]}) แยกกันอยู่คนละจำนวน เพื่อให้มันได้คูณไขว้กันเอง <br>
                    &nbsp;&nbsp;&nbsp; <i>(ให้ {sd[0]} เป็นหลักสิบของตัวคูณ และ {sd[1]} เป็นหลักร้อยของตัวตั้ง)</i><br>
                    👉 <b>หลักถัดมา:</b> นำเลขตัวที่สาม ({sd[2]}) ไปอยู่ฝั่งตัวตั้ง (หลักสิบ) เพื่อให้โดนคูณด้วยเลขที่ใหญ่ที่สุด ({sd[0]})<br>
                    👉 <b>หลักหน่วย:</b> นำเลขตัวที่สี่ ({sd[3]}) วางเป็นหลักหน่วยของตัวคูณ และเลขตัวสุดท้าย ({sd[4]}) วางเป็นหลักหน่วยของตัวตั้ง<br>
                    <b>ขั้นตอนที่ 3: สรุปผลการจัดเรียงและหาคำตอบ</b><br>
                    👉 เราจะประกอบร่างตัวเลขได้เป็น: <b>{best_pair[0]}</b> และ <b>{best_pair[1]}</b><br>
                ...
                if is_challenge:
                    ...
                    👉 ย้ายการบวกเป็นลบ: {wrong_ans} - {B} = {wrong_ans - B} (หมายความว่า {f_s} มีค่าเท่ากับ {wrong_ans - B})<br>
                    👉 ย้ายเศษส่วน(การหาร)เป็นคูณ: นำ {wrong_ans - B} × {A} = {X}<br>
                ...
                if is_challenge:
                    ...
                    👉 ผลบวกแนวกว้าง: 1+2+...+{N} = {N*(N+1)//2}<br>
                    👉 ผลบวกแนวยาว: 1+2+...+{M} = {M*(M+1)//2}<br>
                    👉 นำมาคูณกัน: {N*(N+1)//2} × {M*(M+1)//2} = <b>{total_rect:,} รูป</b><br>
                    <b>ขั้นตอนที่ 2: หาจำนวนสี่เหลี่ยมจัตุรัสทั้งหมด</b><br>
                    👉 ใช้สูตรลดทอนทีละ 1 แล้วคูณบวกกัน: ({N}×{M}) + ({N-1}×{M-1}) + ... ลดไปเรื่อยๆ จนถึง 1<br>
                    👉 คำนวณผลรวมจะได้ทั้งหมด <b>{total_sq:,} รูป</b><br>
                    <b>ขั้นตอนที่ 3: หักล้างกัน</b><br>
                    👉 นำ {total_rect:,} ลบ {total_sq:,} = <b>{total_rect - total_sq:,} รูป</b><br>
                    ...
                    👉 ให้นำขนาดของตารางแต่ละตัวมายกกำลังสอง แล้วนำมาบวกกันตั้งแต่ 1² จนถึง {grid}²<br>
                ...
                if is_challenge:
                    ...
                    <b>ขั้นตอนที่ 2: กำจัดเลข "- {D}" ออกจากวงเล็บใหญ่</b><br>
                    👉 ย้ายข้างไปฝั่งขวา เปลี่ยนลบเป็นบวก: {V1} + {D} = <b>{V2}</b><br>
                    <b>ขั้นตอนที่ 3: กำจัดส่วน "{C}" (ซึ่งหมายถึงการหาร)</b><br>
                    ...
                    👉 นำ {b} ย้ายข้ามเครื่องหมายเท่ากับ จะเปลี่ยนเครื่องหมายจากคูณเป็นหาร: {c} ÷ {b} = <b>{c//b}</b><br>
                    <b>ขั้นตอนที่ 2: หาค่าของตัวเลขใน 🔲</b><br>
                    👉 สมการจะเหลือแค่วงเล็บด้านในคือ: 🔲 + {a} = {c//b}<br>
                    👉 นำ {a} ย้ายไปฝั่งขวา เปลี่ยนบวกเป็นลบ: {c//b} - {a} = <b>{ans_v}</b><br>
                    <b>ตอบ: {ans_v}</b></span>"""
                ...
                if is_challenge:
                    ...
                    👉 ต้องนำเลข {C} มาคูณทั้งตัวเศษและตัวส่วนของก้อนกลาง จะได้เป็น {f_sol_M1}<br>
                    ...
                    👉 ต้องหาว่ามีเลขอะไรที่คูณแม่ {C} แล้วมีผลลัพธ์อยู่ระหว่าง {A} ถึง {D}<br>
                    👉 เลขที่สอดคล้องกับเงื่อนไขนี้คือ: {ans_list}<br>
                    ...
                    👉 ถ้าสมการคือ 🔲 + {a} = {limit_val} ➔ 🔲 จะเท่ากับ {limit_val} - {a} = {limit_val-a}<br>
                    <b>ขั้นตอนที่ 2: หาค่าที่มากที่สุดตามเงื่อนไขของอสมการ</b><br>
                    👉 แต่โจทย์ระบุเครื่องหมาย <b>"ต้องน้อยกว่า"</b> {limit_val} <br>
                    👉 ดังนั้นจำนวนนับที่มากที่สุดที่เป็นไปได้ คือการถอยค่าลงมา 1 แต้ม ➔ {limit_val-a} - 1 = <b>{max_val}</b><br>
                ...
                if is_challenge:
                    ...
                    👉 ตัวตั้งคือ {Z} ลบด้วย {C} ซึ่งลบไม่ได้ จึงต้องไปขอยืมหลักสิบมา 10<br>
                    👉 จะกลายเป็น (10 + {Z}) - {C} = <b>{10+Z-C}</b> (นี่คือตัวเลขกล่องล่างสุด)<br>
                    <b>ขั้นตอนที่ 2: วิเคราะห์หลักสิบ (ตรงกลาง)</b><br>
                    👉 เลข {Y} ถูกยืมไป 1 จึงเหลือเพียง {Y-1} แต่ผลลบยังคงน้อยกว่า 🔲 จึงต้องขอยืมหลักร้อยมาอีก 10<br>
                    ...
                    👉 จะได้สมการ: (🔲 - 1) - {A} = {str_Res[0]} ➔ ย้ายข้าง 🔲 = {int(str_Res[0])} + {A} + 1 = <b>{X}</b> (นี่คือตัวเลขกล่องบนสุด)<br>
                    ...
                    sol = f"""<span style='color:#2c3e50;'><b>วิธีคิดวิเคราะห์แบบ Step-by-Step:</b><br>
                    <b>ขั้นตอนที่ 1: แปลงตารางเป็นสมการแนวนอน</b><br>
                    👉 จากการตั้งคูณ หมายความว่า (จำนวนสองหลักด้านบน) × {n2} = {ans_val}<br>
                    <b>ขั้นตอนที่ 2: ใช้หลักการย้ายข้างสมการเพื่อหาจำนวนนั้น</b><br>
                    👉 ย้าย {n2} ที่กำลังคูณอยู่ ข้ามฝั่งไปเป็นหาร: {ans_val} ÷ {n2} = <b>{n1}</b><br>
                    <b>ขั้นตอนที่ 3: เทียบหาตัวเลขที่หายไป</b><br>
                    👉 จำนวนสองหลักที่คำนวณได้คือ {n1} เมื่อนำไปเทียบกับตาราง ตัวเลขที่หายไปในหลักหน่วยคือ <b>{str_n1[1]}</b><br>
                ...
                items_pair = [
                    ("แตงโม", "สับปะรด", "แอปเปิล", "สตรอว์เบอร์รี"), 
                    ("หมู", "ไก่", "กระต่าย", "นก"), 
                    ("รถบรรทุก", "รถยนต์", "มอเตอร์ไซค์", "จักรยาน")
                ]
                ...
                if is_challenge:
                    ...
                    q = f"ตาชั่งสมดุล 3 เครื่อง ให้ข้อมูลเปรียบเทียบดังภาพต่อไปนี้:<br>{s1_html}{s2_html}{s3_html}<br>อยากทราบว่า <b>{i1} จำนวน {q_mul} {u1}</b> จะมีน้ำหนักสมดุลเท่ากับ <b>{i4}</b> กี่{u4}?"
                    ...
                    👉 เมื่อนำ {i3} ไปแทนที่ {i2} จะได้: 1 × {m1} × {m2} = <b>{m1*m2} {u3} (ของ {i3})</b><br>{sol_s1}
                    <b>ขั้นตอนที่ 2: แทนค่าต่อไปยังตาชั่งที่ 3</b><br>
                    👉 เมื่อนำ {i4} ไปแทนที่ {i3} จะได้: {m1*m2} × {m3} = <b>{m1*m2*m3} {u4} (ของ {i4})</b><br>{sol_s2}
                    <b>ขั้นตอนที่ 3: หาคำตอบตามที่โจทย์ถาม</b><br>
                    👉 โจทย์ถามหา {i1} {q_mul} {u1} ➔ นำปริมาณ {i4} ไปคูณ {q_mul} <br>
                    👉 จะได้: {m1*m2*m3} × {q_mul} = <b>{ans} {u4}</b><br>{sol_s3}
                    <b>ตอบ: {ans} {u4}</b></span>"""
                else:
                    ...
                    q = f"ตาชั่งสมดุล 2 เครื่อง ให้ข้อมูลเปรียบเทียบดังภาพต่อไปนี้:<br>{s1_html}{s2_html}<br>อยากทราบว่า <b>{i1} จำนวน {q_mul} {u1}</b> จะมีน้ำหนักสมดุลเท่ากับ <b>{i3}</b> กี่{u3}?"
                    ...
                    👉 จะได้ความสมดุลใหม่: {i1} 1 {u1} = {m1} × {m2} = <b>{m1 * m2} {u3} (ของ {i3})</b><br>{sol_s1}
                    <b>ขั้นตอนที่ 2: หาคำตอบตามจำนวนที่โจทย์ถาม</b><br>
                    👉 โจทย์ถามหา {i1} จำนวน {q_mul} {u1} ➔ ให้นำความสัมพันธ์ไปคูณด้วย {q_mul}<br>
                    👉 จะได้: {m1 * m2} × {q_mul} = <b>{ans} {u3}</b><br>{sol_s2}
                    <b>ตอบ: {ans} {u3}</b></span>"""
...
if st.sidebar.button(f"{'🚀 สั่งสร้างข้อสอบระดับ Ultimate Challenge!' if is_challenge else '🚀 สั่งสร้างข้อสอบแข่งขันเดี๋ยวนี้'}", type="primary", use_container_width=True):
    with st.spinner("กำลังออกแบบข้อสอบ วาดภาพกราฟิกประกอบ และจัดทำเฉลยแบบ Step-by-Step..."):
        ...
        # ส่งตัวแปร 4 ตัวอย่างถูกต้อง แก้บั๊กจอแดง 100%
